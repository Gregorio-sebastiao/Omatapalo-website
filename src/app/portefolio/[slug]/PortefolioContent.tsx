...
export default function PortefolioContent({ slug }: { slug: string }) {
      ...
      <main>
        <div style={{ position: 'relative', height: 'clamp(320px, 55vh, 640px)', background: '#07101f', overflow: 'hidden' }}>
          ...
          <div className="wrap" style={{ position: 'relative', zIndex: 2, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', paddingBottom: 'clamp(32px,6vh,64px)' }}>
            <Link href="/portefolio" style={{ color: 'rgba(255,255,255,0.6)', fontSize: 12, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', textDecoration: 'none', marginBottom: 16, display: 'inline-flex', alignItems: 'center', gap: 6 }}>← Portfólio</Link>
            <div style={{ display: 'inline-block', background: '#1a396e', padding: '4px 12px', fontSize: 10, fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#fff', marginBottom: 12 }}>{catLabel}</div>
            <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'clamp(2rem, 5vw, 4rem)', color: '#fff', textTransform: 'uppercase', letterSpacing: '-0.03em', lineHeight: 1, margin: 0 }}>{project.title}</h1>
          </div>
        </div>
        <div className="wrap" style={{ paddingTop: 'clamp(40px,7vh,80px)', paddingBottom: 'clamp(48px,8vh,96px)' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 48, alignItems: 'start' }}>
            <div>
              ...
              {lightbox !== null && (
                <div onClick={() => setLightbox(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(7,16,31,0.95)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <button onClick={e => { e.stopPropagation(); setLightbox(i => i! > 0 ? i! - 1 : images.length - 1); }} style={{ position: 'absolute', left: 20, background: 'rgba(255,255,255,0.1)', border: 'none', color: '#fff', fontSize: 28, width: 52, height: 52, borderRadius: '50%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>‹</button>
                  <div onClick={e => e.stopPropagation()} style={{ maxWidth: '90vw', maxHeight: '90vh', position: 'relative' }}>
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={images[lightbox]} alt={`${project.title} ${lightbox + 1}`} style={{ maxWidth: '90vw', maxHeight: '85vh', objectFit: 'contain', display: 'block', borderRadius: 4 }} />
                    <div style={{ textAlign: 'center', marginTop: 12, color: 'rgba(255,255,255,0.5)', fontSize: 12, letterSpacing: '0.1em' }}>{lightbox + 1} / {images.length}</div>
                  </div>
                  <button onClick={e => { e.stopPropagation(); setLightbox(i => i! < images.length - 1 ? i! + 1 : 0); }} style={{ position: 'absolute', right: 20, background: 'rgba(255,255,255,0.1)', border: 'none', color: '#fff', fontSize: 28, width: 52, height: 52, borderRadius: '50%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>›</button>
                  <button onClick={() => setLightbox(null)} style={{ position: 'absolute', top: 20, right: 20, background: 'rgba(255,255,255,0.1)', border: 'none', color: '#fff', fontSize: 20, width: 40, height: 40, borderRadius: '50%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
                </div>
              )}
            </div>
            <div style={{ background: '#f6f8fb', borderRadius: 8, padding: 24, border: '1px solid #e2e8f0' }}>
              ...
              <Link href="/portefolio" style={{ display: 'block', textAlign: 'center', marginTop: 8, padding: '10px', background: '#1a396e', color: '#fff', fontSize: 13, fontWeight: 700, textDecoration: 'none', borderRadius: 6 }}>← Ver todos os projectos</Link>
            </div>
          </div>
        </div>
      </main>
