...
export default function FrArticlesPage() {
  ...
  return (
    <>
      ...
      <main>
        <PageHero
          title="Actualités"
          eyebrow="Groupe Omatapalo · Actualités"
          outlineWord="Actualités"
        ...
        <section style={{ background: '#fff', paddingTop: 'clamp(48px,7vh,80px)', paddingBottom: 'clamp(64px,9vh,100px)' }}>
          <div className="wrap">
            {loading ? (
              <div style={{ textAlign: 'center', padding: '80px 0', color: '#94a3b8', fontSize: 14 }}>Chargement...</div>
            ) : items.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '80px 0', color: '#94a3b8', fontSize: 14 }}>Aucun article publié.</div>
            ) : (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '32px 28px' }} className="media-grid">
                {items.map((item) => (
                  <article key={item.slug} style={{ background: '#fff', border: '1px solid #e8edf5', borderRadius: 4, overflow: 'hidden', display: 'flex', flexDirection: 'column' }} className="media-card">
                    <a href={`/fr/articles/${item.slug}`} style={{ display: 'block', position: 'relative', aspectRatio: '4/3', overflow: 'hidden', background: '#f1f5f9', textDecoration: 'none' }}>
                    ...
                    <div style={{ padding: '20px 22px 24px', display: 'flex', flexDirection: 'column', flex: 1 }}>
                      ...
                      <a href={`/fr/articles/${item.slug}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontFamily: 'var(--font-label)', fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#1a396e', textDecoration: 'none', fontWeight: 700, marginTop: 'auto' }}>
                        Lire Plus »
                      </a>
                    </div>
                  </article>
